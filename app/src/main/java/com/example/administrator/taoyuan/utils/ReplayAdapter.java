...
public class ReplayAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (replay!=null) {
            ...
            // 回复谁，被回复的人可能不存在。
            if(replay.getFatherUser()!=null && replay.getFatherUser().getUserName()!=null&&replay.getFatherUser().getUserName()!=replay.user1.getUserName()) {
...
//                        context,replay.getReplyNickname(),replay.getCommentNickname(),replay.getReplyContent(),position);
...
//                        spannable.getSpanEnd(url), spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            for (URLSpan url : urlspan) {
                if (replay.getFatherUser()!=null) {
                    ...
                    FeedTextViewURLSpan myURLSpan = new FeedTextViewURLSpan(url.getURL(),
                            context,replay.user1.getUserName(),replay.remarkContent,position);
                    stylesBuilder.setSpan(myURLSpan, spannable.getSpanStart(url),
                            spannable.getSpanEnd(url), spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
            }
